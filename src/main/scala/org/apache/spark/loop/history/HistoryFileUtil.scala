...
object HistoryFileUtil {
  ...
  def historyRddAnalysis(locallogpath: String): (String, Map[String, ArrayBuffer[String]]) = {
    ...
    if (files.size > 0) {
      ...
      for (line <- source.getLines) {
        // 取文件夹为key
        val index = line.lastIndexOf(System.getProperty("file.separator"))
        val key = line.substring(0, index)
        val nodeFileName = line.substring(index)
   ...
   * 去除HDFS系统已经不存在的文件
   */
  def del_Hdfs_NotExist(HDFSFileSytem: FileSystem,historyMap: Map[String, ArrayBuffer[String]]) = {
    historyMap.foreach(f => {
      // 文件夹目录不存在，则其下的所有文件都要移除;否则扁历目录把不存在刨除
      if (!HDFSFileSytem.exists(new Path(f._1))) historyMap -= f._1 else historyMap += (f._1 -> (for (eachfile <- f._2 if (HDFSFileSytem.exists(new Path(f._1 + eachfile)))) yield (eachfile)))
    })
  }
   ...
   */
  def logWriter(locallogpath: String, topicLogType: String): FileWriter = {
    try {
