...
object HttpUtil extends Logging {
   ...
   * sent data by group.
   */
  def msgSend(msgList: Array[String]) = {
    val packageList = dataPackageSplit(msgList)
    packageList.foreach(packageMsgList => {
      val msg = messageEdit(packageMsgList)
      //TODO
      // sendPost(serverURL,contentType)(msg)
      logInfo("send to http response:" + msg)
      println("send:" + msg)
    })
  }
   ...
   */
  private def messageEdit(dataList: ArrayBuffer[String]): String = {
   ...
   */
  private def dataPackageSplit(msgList: Array[String]): Array[ArrayBuffer[String]] = {
